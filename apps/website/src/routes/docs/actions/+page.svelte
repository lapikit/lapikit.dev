<script lang="ts">
	...
	let actionsPages = $state<MarkdownWithMetadata[]>([]);

	$effect(() => {
		if (data && data.routes) {
			const list = data.routes.filter((section: SectionPages) => section.key === 'actions');
			actionsPages = list[0].pages.sort((a: MarkdownHead, b: MarkdownHead) =>
...
>
	...
	<div class="grid grid-cols-1 gap-4">
		{#if actionsPages.length === 0}
			<p>{capitalize('No stores found')}</p>
		{/if}
		{#each actionsPages as page, index (index)}
