...
<kit:modal bind:open contain size="lg" classContent="install-modal-content">
	<kit:card id="quick-start-modal">
		<kit:card-title class="install-modal-title"> Install Lapikit </kit:card-title>
		...
		<kit:card-container class="install-modal-body">
			{#if displayMode === 'cli'}
				<kit:card-content class="install-modal-steps">
					<p class="install-modal-step">1. Install Lapikit in your project:</p>
					<LazyRepl lang="sh" content={command[application.pkg_selected]['install-lapikit']} />
				...
				<kit:card-content class="install-modal-steps">
					<kit:accordion>
						...
						<kit:accordion-item
							...
							toggle={accordion.toggle}
						>
							<kit:alert tone="info" class="install-modal-alert">
								{#snippet prepend()}
									<Lightbulb />
								{/snippet}
						...
						>
							<LazyRepl
								lang="sh"
								content={command[application.pkg_selected]['install-eslint-lapikit']}
							/>
						...
						<kit:accordion-item
							...
							toggle={accordion.toggle}
						>
							<LazyRepl lang="sh" content={command[application.pkg_selected]['run-project']} />
						</kit:accordion-item>
					</kit:accordion>
				</kit:card-content>
			{/if}
...
<style lang="scss">
	...
	:global(.install-modal-title) {
		margin-bottom: 0.5rem;
		font-size: 1.5rem;
		font-weight: 900;
	}

	:global(.install-modal-steps) {
		margin-top: 0.5rem;
		margin-bottom: 1rem;
