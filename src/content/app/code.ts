...
export const replVSCodeFiles: Record<string, VsLine[]> = {
	'layout.css': [
		line(0, span('#kit-app', 'css-class'), ' ', span(` ${nbsp} {`, 'accolade')),
		line(2, span('display', 'css-param'), ':', ' ', span(`${nbsp} grid`, 'css-value'), ';'),
		line(
			2,
			span('grid-template-rows', 'css-param'),
			':',
			' ',
			span(`${nbsp} min-content`, 'css-value'),
			' ',
			span(`${nbsp} 1fr`, 'css-value-num'),
			';'
		),
		line(2, span('position', 'css-param'), ':', ' ', span(`${nbsp} relative`, 'css-value'), ';'),
		line(2, span('height', 'css-param'), ':', ' ', span(`${nbsp} 100%`, 'css-value-num'), ';'),
	...
	'+layout.svelte': [
		...
		line(1, span(`import ${nbsp}`, 'import'), ' ', span("'layout.css'", 'path'), ';'),
		blank,
		line(
			1,
			span(`let ${nbsp}`, 'let'),
		...
		line(
			...
			'@',
			' ',
			span(`render ${nbsp}`, 'instruction'),
			' ',
			span('children', 'function'),
			' ',
			span(`() ${nbsp}`, 'function-par'),
			' ',
			span('}', 'accolade')
		),
		line(0, span('</', 'char'), ' ', span('kit:app', 'dom'), ' ', span('>', 'char'))
	...
	'+page.svelte': [
		...
		line(1, span('div:first-child', 'css-dom'), ' ', span(`() ${nbsp} {`, 'accolade')),
		line(2, span('display', 'css-param'), ':', ' ', span(`${nbsp} flex`, 'css-value'), ';'),
		line(2, span('align-items', 'css-param'), ':', ' ', span(`${nbsp} center`, 'css-value'), ';'),
		line(
			2,
			span('justify-content', 'css-param'),
			':',
			' ',
			span(`${nbsp} center`, 'css-value'),
			';'
		),
		line(2, span('max-width', 'css-param'), ':', ' ', span(`${nbsp} 80%`, 'css-value-num'), ';'),
		line(2, span('margin', 'css-param'), ':', ' ', span(`${nbsp} 0 auto`, 'css-value-num'), ';'),
		...
		line(
			...
			span('()', 'accolade'),
			' ',
			span(`${nbsp} > ${nbsp} div ${nbsp}`, 'css-dom'),
			' ',
			span('{', 'accolade')
		),
		line(2, span('text-align', 'css-param'), ':', ' ', span(`${nbsp} center`, 'css-value'), ';'),
		line(1, span('}', 'accolade')),
		blank,
		line(1, span(`p ${nbsp}`, 'css-dom'), ' ', span('{', 'accolade')),
		line(2, span('font-size', 'css-param'), ':', ' ', span(`${nbsp} 32px`, 'css-value-num'), ';'),
		line(
			2,
			span('margin', 'css-param'),
			':',
			' ',
			span(`${nbsp} 10px 0 20px`, 'css-value-num'),
			';'
		),
		line(1, span('}', 'accolade')),
		line(0, span('</', 'char'), ' ', span('style', 'dom'), ' ', span('>', 'char'))
	...
	'Header.svelte': [
		...
		line(
			...
			span('{', 'accolade'),
			' ',
			`${nbsp} createGlobalTheme, useTheme ${nbsp}`,
			' ',
			span('}', 'accolade'),
			' ',
			span('from', 'import'),
		...
		line(
			...
			span('()', 'function'),
			' ',
			span(`${nbsp} => ${nbsp}`, 'instruction'),
			' ',
			span('useTheme(', 'function'),
			' ',
			span('app.active', 'var-name'),
			' ',
			`${nbsp} === ${nbsp} 'dark' ${nbsp} ? ${nbsp} 'light' ${nbsp} : ${nbsp} 'dark'`,
			' ',
			span(')', 'function'),
			' ',
			span('}', 'accolade'),
		...
		blank,
		line(0, span('<', 'char'), ' ', span('style', 'dom'), ' ', span('>', 'char')),
		line(1, span(`img ${nbsp}`, 'css-dom'), ' ', span('{', 'accolade')),
		line(2, span('max-height', 'css-param'), ':', ' ', span(`${nbsp} 32px`, 'css-value-num'), ';'),
		line(1, span('}', 'accolade')),
		line(0, span('</', 'char'), ' ', span('style', 'dom'), ' ', span('>', 'char'))
	],
